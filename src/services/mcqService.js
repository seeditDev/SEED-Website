...
const GAS_ENDPOINT = 'https://script.google.com/macros/s/AKfycbwErf9zlc6vjGOq-idseNeSFknRQckZam_Qev_3k-dn_C65WW0pZBd_pEHC8zhCMkcW/exec';
...
class MCQService {
    ...
    /**
     * Save MCQ result to Google Sheets via Google Apps Script
     * @param {object} resultData - Result data object
     * @returns {Promise<{success: boolean}>}
     */
    static async saveResultToSheets(resultData) {
        try {
            const payload = {
                action: 'submitMCQ',
                ...
                // send both totalQuestions and total for compatibility with GAS
                ...
                timeTaken: resultData.timeTakenFormatted || this.formatTime(resultData.timeTaken || 0),
                timeStarted: resultData.timeStartedISO || timeService.getNow().toISOString(),
                timeEnded: resultData.timeEndedISO || timeService.getNow().toISOString(),
                submittedAt: resultData.submittedAtISO || timeService.getNow().toISOString(),
                autoSubmitted: resultData.autoSubmitted || false,
                autoSubmitReason: resultData.autoSubmitReason || '',
                // Proctoring data
                violationCount: resultData.violationCount || 0,
                totalNoFace: resultData.totalNoFace || 0,
                totalMultipleFaces: resultData.totalMultipleFaces || 0,
                violations: JSON.stringify(resultData.violations || []),
                // explicit field matching GAS buildRowData's violationsDetails
            ...
            const response = await fetch(GAS_ENDPOINT, {
                method: 'POST',
                mode: 'no-cors', // GAS requires no-cors for web apps
                headers: {
    ...
    /**
     * Save in-progress MCQ data to Google Sheets
     * @param {object} progressData
     */
    static async saveProgressToSheets(progressData) {
        const payload = {
            action: 'syncMCQProgress',
            timestamp: progressData.timestamp || timeService.getNow().toISOString(),
            rollNumber: progressData.rollNumber || '',
            name: progressData.name || '',
            email: progressData.email || '',
            college: progressData.college || '',
            year: progressData.year || '',
            department: progressData.department || '',
            testID: progressData.testID || '',
            testName: progressData.testName || 'Unknown Test',
            ...
            timeTaken: progressData.timeTakenFormatted || this.formatTime(progressData.timeTaken || 0),
            timeStarted: progressData.timeStartedISO || timeService.getNow().toISOString(),
            timeEnded: progressData.timeEndedISO || '',
            submittedAt: progressData.submittedAt || '',
            autoSubmitted: progressData.autoSubmitted || false,
            autoSubmitReason: progressData.autoSubmitReason || ''
        };
    ...
    /**
     * Mark Firestore document as synced to Sheets
     * @param {string} email - Student email
     * @param {string} testID - Test ID
     * @param {string} college - College name
     * @param {string} year - Academic year
     ...
     * @returns {Promise<void>}
     */
    static async markSyncedToSheets(email, testID, college, year, department) {
        try {
            const docPath = `colleges/${college}/years/${year}/departments/${department}/students/${email}/mcq_results/${testID}`;
            const docRef = doc(db, docPath);
    ...
    static async syncUnsyncedResults() {
        try {
            ...
            for (const result of unsynced) {
                try {
                    /*
                    // Try to sync to Sheets (Commented out)
                    await this.saveResultToSheets(result);

                    // Mark as synced in Firestore
                    await this.markSyncedToSheets(
                        result.email,
                        result.testID,
                        result.college,
                        result.year,
    ...
    static async submitMCQResult(resultData) {
        ...
        try {
            ...
            /*
            // --- GOOGLE SHEETS SAVE LOGIC (COMMENTED OUT) ---
            // Step 2: Save to Google Sheets (Non-blocking)
            this.saveResultToSheets(resultData).then(async () => {
                // Mark as synced in Firestore
                if (firestoreSuccess) {
                    await this.markSyncedToSheets(
                        resultData.email,
                ...
                console.log('[MCQService] ✅ Google Sheets save successful');
            }).catch(sheetsError => {
                console.error('[MCQService] ⚠️ Google Sheets save failed:', sheetsError);
